...
export function createOverlay<T extends Dict>(
  ...
  const getSnapshotForStore = () => {
    const nextSnapshot: OverlaySnapshotEntry<T>[] = Array.from(map.entries()).map(([id, props]) => ({id, props}))
  ...
  function Viewport() {
    const overlays = React.useSyncExternalStore(subscribe, getSnapshotForStore, getSnapshotForStore)
    return (
      <>
...
const shallowEqualEntries = <T extends Dict>(a: OverlaySnapshotEntry<T>[], b: OverlaySnapshotEntry<T>[]) => {
